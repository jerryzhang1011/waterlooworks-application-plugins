...
5. Upload and verify:
   ...
   - **Verify the uploaded cover letter is selected — it may or may not auto-select.** After the dialog closes the new row appears; WaterlooWorks sometimes auto-selects it and sometimes leaves the radio unselected, and the parent application's `Submit` stays disabled until a cover letter is chosen. Check the cover-letter radio's `checked` state in the DOM; if it is unchecked, click the uploaded cover letter's radio/row and re-verify it is `checked` (and that no other cover-letter option is selected) before handing back. A finished-looking upload that still blocks submission is almost always this missing selection.
