...
# Signals in markdown that a skill depends on an MCP server / external connection
# we cannot install from the CLI. name -> (compiled regex, setup hint).
MCP_SIGNALS = [
    (
        "Claude in Chrome (MCP browser automation)",
        re.compile(r"claude\s+in\s+chrome|mcp__claude_in_chrome|logged-in chrome session", re.I),
