...
A plugin that automates the WaterlooWorks application workflow for **any University of Waterloo student — any program, any work term.** It drives your own logged-in Chrome session to scrape postings, write tailored cover letters, upload documents, and submit Full-Cycle Service applications. Ships for both **Claude Code** (`.claude-plugin/`) and **Codex** (`.codex-plugin/`) from the same skills.
