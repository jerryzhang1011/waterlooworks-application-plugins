...
- **New dependencies are picked up automatically.** Detection is driven by what the skills
  ...
  gains a new tool, this check will flag it without anyone updating a hardcoded list. If a
  brand-new package's import name differs from its pip name and isn't in the script's small
  mapping table, add it to `PIP_NAME_MAP` in `scripts/check_skill_deps.py`.
